...
class GeneratorDriver(_FIREQDriver):
    ...
    def print_description(self):
        print(
            "SampleMemoryAddressWidth: "
            + str(self.SampleMemoryAddressWidth)
            + ", address width of the envelope memory (word/IQSample aligned)"
        )
        print(
            "ChannelSampleMemoryDepth: "
            + str(self.ChannelSampleMemoryDepth)
        ...
        print(
            "TotalSampleMemorySegmentDepth: "
            + str(self.TotalSampleMemorySegmentDepth)
        ...
        print(
            "MemoryMappedFifoSegmentDepth: "
            + str(self.MemoryMappedFifoSegmentDepth)
            + ", memory mapped FIFO segment depth (bytes)"
        )
        print(
            "SeedLfsrWidth: " + str(self.SeedLfsrWidth) + ", width of lsfr seed and memory mapped FIFO entries (bits)"
        )
    ...
    def reset_wave_memory_dict(self):
        ...
        for address in range(
            self.TotalSampleMemorySegmentDepth, self.TotalSampleMemorySegmentDepth + self.WaveMemorySegmentDepth, 4
        ):
    ...
    def set_trigger_channel(self, channel, ttype):
        ...
        control = _set_bits(
            self.AxiLiteInterfaceMMIO.read(self.ctrl * 4),
            selector * self.TriggerChannels,
            self.TriggerChannels,
            trigger_mask,
        )
    ...
    def add_envelope_to_envelope_memory(
        ...
        if envelope_size % self.NumberOfChannels != 0 and not (for_interpolation):
            print(
                "error, envelopes not marked for interpolation must have a number of sample divisible by the generator parlallism"
            )
    ...
    def create_wave_definition_word(
        ...
        # set the gain
        wavedef = wavedef | (
            real_gain << (2 * (self.SampleMemoryAddressWidth + self.FractionalPrecision) + self.DurationWidth)
        )
    ...
    def add_wave_in_wave_memory(self, wave_definition: int, wave_name: str):
        ...
        for i in range(4):
            self.AxiFullInterfaceMMIO.write(
                (self.TotalSampleMemorySegmentDepth + i * 4 + address), (wave_definition >> (i * 32)) & 0xFFFFFFFF
            )
