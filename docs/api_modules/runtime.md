...
* **Startup Sequence (`main()`)**: 
  1. Requests the `.bit` overlay file path relative to the directory containing `start_server.py` (defaulting to `overlay.bit`).
  ...
  4. Instantiates `FIREQServer(ol_filepath, host, port, auth_token=auth_token)` and calls `server.start()`.
