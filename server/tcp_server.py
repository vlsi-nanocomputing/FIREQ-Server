...
from .message_handler import MessageHandler, SweepPointResult
        ...
        try:
            if cmd == "upload_envelopes":
                ...
                def flush_binary_batch() -> None:
                    ...
                    self.logger.debug(f"Sent binary-only batch {batch_index[0]} with {len(binary_buffer)} points")
                ...
                def on_point(r: SweepPointResult) -> None:
                    ...
                    if not header_sent[0]:
                        points_metadata = [{"point_index": i, "variables": p} for i, p in enumerate(sweep_points_plan)]
                        header_payload = {
                            "type": "sweep_header",
                            "cmd": cmd,
                        ...
                        self.queue_out.put({"type": "sweep_header", "metadata": header_payload})
                        header_sent[0] = True
                ...
                # Execute sweep
                status = self.handler.run_sweep(msg, on_point=on_point, stop_event=self._stop_event, on_plan=on_plan)
