    ...
    def _validate_buffer_capacity(
        ...
        if total_requested_bits > usable_fifo_bits:
            if mode == "accumulated":
                ...
                requested_samples = (
                    samp_per_shot * shots_per_exp
                    if mode == "decimated"
                    else samp_per_shot * shots_per_exp * parallelism
                )

                hint_lines = []
                if samp_per_shot > max_samp_per_shot:
                    hint_lines.append(
                        f" samp_per_shot too large: {samp_per_shot} > {max_samp_per_shot} (for {shots_per_exp} shots)"
                    )
                if shots_per_exp > max_shots:
                    hint_lines.append(
                        f"  shots too large: {shots_per_exp} > {max_shots} (for {samp_per_shot} samp/shot)"
                    )
